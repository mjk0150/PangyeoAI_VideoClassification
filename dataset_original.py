...
class VideoDataset(Dataset):
    ...
    def __init__(self, root_dir, dataset='ucf101', split='train', clip_len=16, preprocess=False, size=(171, 128, 112), autoaug=False):
        ...
        if dataset == "ucf101":
            if not os.path.exists(label_dir + '/ucf_labels.txt'):
                with open(label_dir + '/ucf_labels.txt', 'w') as f:
                    for id, label in enumerate(sorted(self.label2index)):
                        f.writelines(str(id+1) + ' ' + label + '\n')

        elif dataset == 'hmdb51':
            if not os.path.exists(label_dir + '/hmdb_labels.txt'):
                with open(label_dir + '/hmdb_labels.txt', 'w') as f:
                    for id, label in enumerate(sorted(self.label2index)):
                        f.writelines(str(id+1) + ' ' + label + '\n')

        elif dataset == 'kids':
            if not os.path.exists(label_dir + '/kids_labels.txt'):
...
class TestDataset(Dataset):
    def __init__(self, root_dir, dataset='kids', clip_len=16, size=(171, 128, 112), preprocess=False):
        ...
        for fname in os.listdir(self.output_dir):
            self.fnames.append(os.path.join(self.output_dir, fname))

        print(f'Number of test set videos: {len(self.fnames)}')
    ...
    def preprocess(self):
        print('Test dataset preprocessing start.')
        test = [name for name in os.listdir(self.root_dir)]
